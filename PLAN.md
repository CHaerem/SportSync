...
| WP-96 | Flerbruker-splitten (interests → katalog) — GATE for eksterne testere | 0G | portmåling | 🔬 PR: server-kompasset byttet fra eierens interests.json til ny `catalog.json` (tier1 wholesale-sporter + tier2 entitets-langhale; JSON-schema + validering). `build-events` `isRelevant(interests)` → `isCovered(catalog)`; personlig presisjon fjernet fra server, eies av klient-linsen alene. mustWatch/events.ics beholdt som eier-artefakt (dokumentert). Vektorer IKKE re-frosset — linsen er uendret; `relevant` pinner nå klient-linsen (FeedCompiler.isRelevant), `isCovered` er server-only (DIVERGENCES §6). To-profil-test beviser disjunkte profiler får meningsfulle events fra samme katalog-feed. Agent-prompter (research/verify/coverage-critic/editorial) retter seg mot katalogen; editorial nøytral tavle-brief. `docs/data/interests.json` avpublisert; web «Dette dekker vi» viser katalogen (eierens personlige visning = iOS). Dokumenterte valg: build-ics (eier-artefakt), rediger/edit (be-om-dekning + TODO WP-23), build-entities/fetch-results/football/detect-coverage-gaps → katalog m/ fallback. 542 tester grønne |
