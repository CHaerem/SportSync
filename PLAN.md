...
| WP-31 | Naturlig onboarding (P310 «definere») | 0B | WP-16.4,WP-05 | 🔬 PR åpnet — rolig førstegangsflyt i `ios/Zenji/Onboarding/`: velkomst m/ personvern-øyeblikk → samtale-steg (gjenbruker `AssistantViewModel`/`InterestAssistant.interpret`, diff-bekreft, voksende «Følger nå») → hurtigvalg-steg (kuraterte norske startpakker fra entitets-indeksen, golf/sykkel `throughNorwegians`, cold-start-trygge) → landet i fylt agenda (`EffectiveInterests` re-kompilerer live). Førstegangs-deteksjon + persistent flagg (`OnboardingGate`, `@AppStorage`), re-kjørbar fra «Hva jeg følger»; hopp-over → rolig tom-tilstand m/ peker til `»_`. Ikke en parallell inntasting. 368/368 iOS-tester (+12), 373/373 JS urørt, begge schemes bygger, ZenjiDeviceDev bygget/signert/installert på iPhone 16 Pro; skjermbilder begge temaer i `ios/docs/design-v2/onboarding-{welcome,converse,quickpicks,landed}-{dark,light}.png` |
