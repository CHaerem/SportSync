...
| WP-17 | 💰 TestFlight-oppsett | 0B | WP-14 | 🔬 pågår 18.07 — Apple Developer-enrollment GODKJENT: gratis-teamet ble konvertert in place (samme team-ID 9LVCB72DT8); `app.sportivista.ios` + widget-id + App Group + iCloud-container registrert via provisjoneringsbygg; device-bygget signerer nå fulle entitlements, embedder widgeten og kompilerer med `-D SPORTIVISTA_CLOUDKIT` (CloudKitProfileSync aktiv) — installert på eierens iPhone. GJENSTÅR: app-record i ASC (eier oppretter — riktig bundle-id synlig nå), ASC API-nøkkel fra eier, arkiv + TestFlight-opplasting, intern testgruppe |
