...
| WP-134 | Visningsbugs: tekst-forskyvning/feil størrelse (eier-dogfooding 20.07) | 0I | — | 🔬 Dynamic Type-overlapp ved AX: agenda-raden (`AgendaView`) reflow-er VERTIKALT ved `dtSize.isAccessibilitySize` (tid/vindu + sport-symbol egen linje OVER full-bredde-tittel; ikke-AX horisontal-layout pikselidentisk), tittel aldri avkuttet (lineLimit nil ved AX); hardkodede glyf-`.frame(width:)` → `@ScaledMetric` (SportSymbolView + FollowedListView ×3 + OnboardingView-markører); TimeColumn slipper `.fixedSize` ved AX (flerdags-vindu wrapper). CI-gate `ios-dynamic-type-gate` utvidet: flagger SF-symbol/glyf i hardkodet frame-bredde (null falske positiver: prikk/QR/etikett-kolonne fritatt). Verifisert visuelt AX2/3/5 dark+light (lens + onboarding-landed) + std pikselidentisk |
