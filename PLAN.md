...
| WP-181 | Briefen som navngitt rituale («Morgenbriefen»/«Kveldsbriefen») | 0L | 0K WP-174 | ✅ #415 merget 23.07 — #415 PR åpen — branch wp-181-briefen-ritual — WP-174-briefen NAVNGITT som rituale (motoren urørt, WP-174s frosne kontrakt). (1) Stille seksjonstittel «Morgenbriefen»/«Kveldsbriefen» (bestemt form, grå footnote-semibold uppercase) over briefen på BEGGE flater — iOS Nyheter-slot (`board.briefTitle`), web hero (`heroBriefTitle`), kun når en ekte brief vises (aldri over fallback-linja). (2) Tidsgrense 15:00 Oslo, ÉN delt definisjon — `BriefRitual.eveningHour` ↔ `SS_BRIEF_EVENING_HOUR`/`ssBriefRitualName`, tvilling-testet. (3) Opt-in daglig lokal varsel «Morgenbriefen er klar» (kropp «Åpne når du vil.») ~06:45 Oslo (ETTER 06:30-editorialen), GENERISK — aldri resultat/spoiler; `BriefAlertPreference` (per-enhet, AV default) + `BriefNotificationPlanner` (repeterende UNCalendarNotificationTrigger); bryter i Deg › APP. (4) Widget-speiling: medium-widgetens ENE følgelinje er briefen om morgenen (ellers «siste resultat»), PRE-RENDRET (`WidgetBriefSnapshot`, som `widget-result.json`) + dag-gatet, ingen ny familie. DESIGN.md § Briefen som rituale + Deg-APP + Nyheter-seksjon oppdatert. Tester: JS 1174 (+5 web brief-tittel/ritual), iOS +BriefRitualTests/BriefNotificationPlannerTests/widget-brief-fixtures/NewsBoard-briefTitle/NewsModel-snapshot (0 feil), 13/13 gylne feed-vektorer + brief-vektorer bit-like, alle 4 schemes bygger. Skjermbilder: web+iOS dark/light |
...
| WP-187 | Hygiene: flaky konkurranse-tester + perf-port-CI + avatar-proporsjoner | 0L | — | ✅ #414 merget 23.07 — branch wp-187-hygiene (PR åpen) — tre urelaterte fikser i én PR (sanksjonert unntak fra én-WP-regelen). (1) **AgendaReloadConcurrencyTests last-robust**: rotårsak var at HVER `computeReloadSync` bygde `EntityIndex` over hele 3 659-entiteters verdensregister på nytt (`init` kompilerer én `NSRegularExpression` per term → tusenvis av regex-kompileringer), så én compute tok ~9 s i simulatoren (`test_guard_trips…` målt 9,2 s), ballongerte til titalls sekunder under parallell xcodebuild-last og kostet agenter re-kjøringer. Klassen tester HVOR (off-main) og HVOR OFTE (≤2-recompile-koalescering) compute kjører — entitets-tall-uavhengige garantier — så den fikk en egen liten entitets-fixture (7 poster) i stedet for verdensregisteret; koalescerings-/last-vins-/main-tråd-vakt-semantikken uendret (fortsatt eksakt `== 2`, tilstandsbasert `awaitReloadsQuiescent`). Compute falt fra ~9 s til ~0,18 s; **10/10 grønn i loop under load-avg topp 39,7** (to samtidige clean-build-loops). (2) **Perf-portens absolutte veggklokke**: `test_buildSections_scalesLinearly…` felte PR #410 på «496 ms > 150 ms» på GitHubs macOS-runner (taket var dimensjonert på lokal M-serie). RATIO-vakten (~2× lineær vs. ~4× kvadratisk, tak 3,0) er den maskin-UAVHENGIGE O(n²)-sentinelen og er nå ENESTE harde port; den absolutte tiden er nedgradert til logget advarsel (aldri en assertion) — en hard veggklokke kan ikke skille «treg runner» fra «regresjon». Bevist: syntetisk kvadratisk regresjon (injisert O(events×entities) i den målte regionen) → ratio 3,76–3,86× på alle 3 forsøk → XCTFail (bevis-testen fjernet før commit); ren kjøring ratio 2,11× → grønn. (3) **Brann-ovalens proporsjoner** (`EntityAvatarView.swift` + `docs/css/entity-logo.css` — CSS-tvillingen til `entity-avatar.js`-markupen): brede/ovale merker klemtes/rant utenfor 24 pt-boksen (web `width/height:auto` lot 96 px-kilden OVERFLYTE — Man Utd rant forbi boksen; iOS var allerede bundet av `.scaledToFit`). Nå deterministisk aspect-fit inn i boksen + litt luft på begge flater (web `width/height:100%; object-fit:contain` + `padding:2px` border-box; iOS `.padding(side*0.08)`), ingen beskjæring/omfarging (WP-186 uendret), samme ytre 24 pt-boks. Verifisert visuelt Brann+Man Utd+Bodø/Glimt dark+light på begge flater. Full iOS-suite 846/846 + 4 schemes + 14 gylne vektorer bit-like; `npx vitest run --maxWorkers=1` 1170/1170 grønn |
