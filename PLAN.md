...
| WP-92 | Relevans-gaten (chess/esport + iOS-låssteg) | 0G | – | 🔬 PR åpen — `isRelevant` omskrevet (server + iOS FeedCompiler i lås-steg): chess/esport ut av default-followBroadly og **entitets-gatet** (sport-scopet match; ingen norwegian/favorite/importance/ai-research-blankett), og ai-research-autopasset scopet (må også være followBroadly-sport ELLER matche entitet). Live-diff: KUN 2 events falt av tavla — «Sant Martí» (bevist brudd) + «Sjakk-NM eliteklassen» (grensetilfelle: verken Carlsen/Tari, dokumentert i PR); Carlsen-sjakk + 100 Thieves-CS2 består. Gylne vektorer re-frosset bevisst (12 endret, ny vektor 14 for gaten + sport-scope-forsvaret; DIVERGENCES §4/§5). 526 JS-tester + iOS-unit-suite grønn (FeedVectorTests bit-like), alle fire schemes bygger. research.md: sjakk-elite-terskel + CS2-kun-100T |
