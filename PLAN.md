...
| WP-95 | Deltakelses-ferskhet (cut/trekning — eier-funn) | 0G | – | 🔬 PR — golf-berikelsen setter `norwegianPlayers[].status` («røk cutten»/«trakk seg»/«diskvalifisert») fra ESPNs core-API competitor-status (`…/competitors/{id}/status` → `type.name` STATUS_CUT/WITHDRAWN/DISQUALIFIED + `displayValue` CUT/WD/DQ — den robuste kilden; det lette site.api-scoreboardet eksponerer IKKE cut mid-turnering, verifisert The Open R3). Én billig henting per fulgt spiller (scoreboard-competitor-id = core-API-id), kun for pågående turneringer. En ute-spiller får ingen tee-tid/featured-group. Skjemaet støttet `status` alt (WP-04, additivt). verify.md: deltakelses-sjekk som eget kontraktspunkt (samme alvor som tid/kanal); editorial.md: hard regel mot deltaker-påstand vs. stale status; grader-rubrikk: hard failure #7 (påstand vs. motstridende status). Web: detail.js + followed.js viser rolig status i stedet for «pågår nå»/tee. iOS uendret (LensRenderer rendrer `status` alt ordrett). Lokalt verifisert: Hovland ⇒ «røk cutten», overlever build-events-rebuild, validate rent; 536 tester grønne |
