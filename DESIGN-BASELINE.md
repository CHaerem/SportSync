# Zenji designsystem — Apple-native baseline (UTKAST)

> **Status: UTKAST til eierens vurdering.** Dette erstatter intensjonen i
> `DESIGN.md` (v2, Tekst-TV) med et **Apple-native fundament**: systemfont,
> semantiske system-farger, SF Symbols, native lister/sheets/navigasjon — med
> **amber som eneste aksent-token**. `DESIGN.md` (v2) er fortsatt den levende
> kontrakten til koden følger etter; når baselinen er implementert promoteres
> denne filen til `DESIGN.md`. (Den tidligere `DESIGN-v3-DRAFT.md` — Tekst-TV +
> native mekanikk — er utdatert av denne og skal slettes.)
>
...
- **Web (docs/)** og **widget** følger de SAMME tokenene (semantiske farger +
  Dynamic Type-analog med `rem`/`clamp`). Widgeten er en miniatyr av samme språk.
