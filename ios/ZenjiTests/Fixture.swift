...
//  ZenjiTests
//
//  WP-11: loads the checked-in JSON snapshots under ZenjiTests/Fixtures/ —
...
/// Anchors `Bundle(for:)` lookups at the ZenjiTests bundle, where XcodeGen's
/// `resources: [ZenjiTests/Fixtures]` entry (project.yml) copies the fixture
/// files.
...
enum Fixture {
    static func url(_ name: String, ext: String = "json") -> URL {
        ...
        guard let url = bundle.url(forResource: name, withExtension: ext) else {
            fatalError("Missing fixture \(name).\(ext) — check ZenjiTests/Fixtures/ and project.yml resources.")
        }
        return url
    }
