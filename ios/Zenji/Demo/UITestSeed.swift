...
//  Zenji
//
//  WP-70 — the deterministic launch harness the XCUITest suite (ZenjiUITests)
...
//  under any `ZENJI_DEMO` value (so no "Apple Intelligence off" banner), and
//  skips the network sync for `uitest` exactly as it does for `lens`.
//
//  Two launch states, chosen by the `ZENJI_UITEST_STATE` environment variable
...
//
//  Never compiled into a release build (`#if DEBUG`), and lives in Zenji/Demo/
//  (WP-48) — only the app targets' `path: Zenji` picks that folder up; the
...
enum UITestSeed {
	...
	/// The `ZENJI_DEMO` value that selects this harness.
	static let demoMode = "uitest"

	/// Launch states, from `ZENJI_UITEST_STATE`.
	enum State: String {
		case onboarding
		case agenda
	}
	...
	static var isRequested: Bool {
		ProcessInfo.processInfo.environment["ZENJI_DEMO"] == demoMode
	}

	/// The requested launch state (defaults to `.agenda`).
	static var requestedState: State {
		State(rawValue: ProcessInfo.processInfo.environment["ZENJI_UITEST_STATE"] ?? "") ?? .agenda
	}
