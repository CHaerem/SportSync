...
//  contract parse through the shared SportivistaJSON decoder strategy (in
...
final class ForwardCompatibilityTests: XCTestCase {
    ...
    func testEntityArrayDecodesWithUnknownField() throws {
        ...
        let entities = try SportivistaJSON.decoder.decode([Entity].self, from: json)
    ...
    func testWithFractionalSecondsParses() throws {
        ...
        let holder = try SportivistaJSON.decoder.decode(DateHolder.self, from: json)
    ...
    func testUnparsableDateStringThrows() {
        ...
        XCTAssertThrowsError(try SportivistaJSON.decoder.decode(DateHolder.self, from: json))
    }
}
