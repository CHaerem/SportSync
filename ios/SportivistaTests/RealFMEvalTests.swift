...
//    TEST_RUNNER_SPORTIVISTA_EVAL_CATEGORY=canon,command   → bare de kategoriene
//    TEST_RUNNER_SPORTIVISTA_EVAL_CASE=canon-1,cmd-theme    → bare de case-id-ene
//  (xcodebuild speiler TEST_RUNNER_<X> til miljøvariabelen <X> i testprosessen,
//  samme mekanikk som TEST_RUNNER_SPORTIVISTA_REALFM_EVAL over.) Rapportformatet er
...
final class RealFMEvalTests: XCTestCase {
	...
	func test_realFM_corpusReport() async throws {
		...
		//   TEST_RUNNER_SPORTIVISTA_REALFM_EVAL=1 xcodebuild test … -only-testing:SportivistaTests/RealFMEvalTests
		let env = ProcessInfo.processInfo.environment
		guard env["SPORTIVISTA_REALFM_EVAL"] == "1" else {
			throw XCTSkip("Opt-in eval — sett TEST_RUNNER_SPORTIVISTA_REALFM_EVAL=1 for å kjøre mot ekte FM.")
		}
		...
		let categoryFilter = Self.csvSet(env["SPORTIVISTA_EVAL_CATEGORY"])
		let caseFilter = Self.csvSet(env["SPORTIVISTA_EVAL_CASE"])
		...
		XCTAssertFalse(cases.isEmpty, "Filteret matchet ingen cases — sjekk SPORTIVISTA_EVAL_CATEGORY/SPORTIVISTA_EVAL_CASE.")
