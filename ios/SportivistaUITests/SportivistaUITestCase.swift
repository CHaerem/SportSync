//
//  SportivistaUITestCase.swift
//  SportivistaUITests
//
//  WP-70 — shared scaffolding for the XCUITest suite. Every flow launches the
//  app against the deterministic `SPORTIVISTA_DEMO=uitest` harness (UITestSeed in the
...
class SportivistaUITestCase: XCTestCase {
	...
	/// Launch the app in a known state. `state` maps to `SPORTIVISTA_UITEST_STATE`:
	...
	func launchApp(state: String) -> XCUIApplication {
		...
		app.launchEnvironment["SPORTIVISTA_UITEST_STATE"] = state
		app.launch()
		return app
	}
