...
- **`ios-release.yml`** — the TestFlight release lane (WP-17): from a clean checkout it archives, cloud-signs, and uploads the app to App Store Connect, then records the upload (`scripts/record-testflight.js`; build number from `scripts/next-testflight-build.js` — ASC is the source of truth) and kicks the static pipeline to publish `app-version.json`. Triggers: manual (`workflow_dispatch`), an `ios-v*` tag, or an **hourly schedule** (`35 6-22`, WP-137) that closes the device loop — a cheap `detect` job (ubuntu) builds only when `ios/` changed since the last recorded TestFlight stamp (`scripts/config/testflight.json`), so the owner always dogfoods HEAD instead of a build days behind the fixes; a burst of iOS merges collapses to ONE build on the next tick, and quiet hours are free skips. Needs the three `ASC_*` secrets from an **ADMIN** ASC key (App Manager can't cloud-sign).
