...
- **Tournament Brackets**: Generic bracket visualization supports single elimination (Champions League, World Cup) and double elimination (CS2 esports). `build-events.js` extracts bracket data from curated configs → `brackets.json`. Client renders horizontal tree with connector lines, team logos, match times, focus team highlighting
...
4. **`fetch-standings.js`** fetches PL table, golf leaderboards (with player headshot URLs from ESPN CDN), F1 driver standings from ESPN
...
12. **`build-events.js`** auto-discovers sport files from `docs/data/*.json` by convention (any file with `{ tournaments: [...] }`) and curated configs from `scripts/config/*.json`, merges all into `events.json`. Also extracts tournament bracket data from configs with `bracket` fields → `brackets.json`
...
19. **Client-side** loads `events.json` + `featured.json` + `standings.json` + `recent-results.json` + `brackets.json`, renders editorial dashboard with collapsible results band. **Component blocks** (`match-result`, `match-preview`, `event-schedule`, `golf-status`) resolve against pre-loaded data for logos, scores, times, and standings — falling back to `_fallbackText` when data is unavailable. **Tournament brackets** render as horizontal trees with connector lines, team logos, match times, and focus team highlighting — supports both single elimination (CL) and double elimination (CS2). **Day navigator** lets users browse past/future days, async-loads `featured-{date}.json` for date-specific briefings
...
- `npm test` - Run all tests (vitest, 2110 tests across 67 files)
...
│   ├── asset-maps.js       # Team logos + golfer headshot URLs (fallback for ESPN CDN auto-headshots)
│   ├── sport-config.js     # Sport metadata (10 sports incl. Olympics, biathlon, nordic, alpine)
...
└── data/                   # Pre-fetched API data (auto-generated)
    ...
    ├── brackets.json       # Tournament bracket data (extracted from curated configs by build-events.js)
...
│   ├── esports-cs2-2026.json # CS2 tournament configs with bracket data (DraculaN, etc.)
...
tests/                      # 2110 tests across 67 files (vitest)
...
| **Esports data** | Tournament bracket visualization working (double + single elimination, team logos, match times). Learned scraper recipe extracts CS2 matches from Liquipedia (zero LLM cost). Remaining gaps: live score updates for esports matches, logos for smaller CS2 teams (HLTV CDN blocks server-side access). | Low |
