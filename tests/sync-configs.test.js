...
// --- Tournament-aware needsResearch ---

describe("syncConfigs() tournament-aware needsResearch", () => {
	it("does NOT flag needsResearch when config has active tournaments", () => {
		const now = new Date("2026-02-26T12:00:00Z");
		writeJson(path.join(configDir, "esports-cs2-2026.json"), {
			name: "CS2 Tournaments",
			sport: "esports",
			autoGenerated: true,
			events: [], // Empty after pruning
			tournaments: [
				{
					id: "active-t",
		...
		const result = syncConfigs({ configDir, archiveDir, now });
		expect(result.flagged).not.toContain("esports-cs2-2026.json");

		// Verify the config was NOT marked needsResearch
		const config = JSON.parse(
			fs.readFileSync(path.join(configDir, "esports-cs2-2026.json"), "utf-8")
		);
		expect(config.needsResearch).toBeUndefined();
	});

	it("flags needsResearch when tournaments are all expired", () => {
		const now = new Date("2026-02-26T12:00:00Z");
		writeJson(path.join(configDir, "esports-old.json"), {
			name: "Old Esports",
			sport: "esports",
			autoGenerated: true,
			events: [],
		...
		const result = syncConfigs({ configDir, archiveDir, now });
		expect(result.flagged).toContain("esports-old.json");
	});
});
