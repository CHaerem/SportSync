...
describe("v2 workflows", () => {
	it("exactly the expected agent workflows exist (old autopilot removed)", () => {
		...
		for (const f of ["static-pipeline.yml", "research-agent.yml", "verify-agent.yml", "editorial-agent.yml"]) {
	...
	it("agent workflows have concurrency guards", () => {
		for (const f of ["research-agent.yml", "verify-agent.yml", "editorial-agent.yml", "static-pipeline.yml"]) {
