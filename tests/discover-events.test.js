...
describe("findResearchTasks()", () => {
	...
	it("skips autoGenerated configs that failed discovery within 24h (backoff)", () => {
		const now = new Date("2026-04-03T12:00:00Z");
		const configs = [
			{
				filename: "esports-cs2-2026.json",
				config: { name: "CS2", autoGenerated: true, events: [] },
			},
		];
		const discoveryLog = {
			runs: [
				{
					timestamp: "2026-04-03T10:00:00Z", // 2h ago — within 24h cooldown
					tasks: [
						{ type: "new-config", target: "esports-cs2-2026.json", outcome: "failed", error: "ETIMEDOUT" },
					],
				},
			],
		};
		const tasks = findResearchTasks(configs, null, now, null, discoveryLog);
		expect(tasks).toHaveLength(0);
	});

	it("retries autoGenerated configs after 24h cooldown expires", () => {
		const now = new Date("2026-04-03T12:00:00Z");
		const configs = [
			{
				filename: "esports-cs2-2026.json",
		...
		const discoveryLog = {
			runs: [
				{
					timestamp: "2026-04-02T08:00:00Z", // 28h ago — cooldown expired
					tasks: [
						{ type: "new-config", target: "esports-cs2-2026.json", outcome: "failed", error: "ETIMEDOUT" },
					],
				},
		...
		const tasks = findResearchTasks(configs, null, now, null, discoveryLog);
		expect(tasks).toHaveLength(1);
		expect(tasks[0].filename).toBe("esports-cs2-2026.json");
	});
