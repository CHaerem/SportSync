...
- [PENDING] [MAINTENANCE] **recapHeadline 4th-tier sport filter too strict** — The single-team 4th-tier matching in `matchRssHeadline()` checks `item.sport === "football"` but NRK/TV2 general sport feeds are tagged `sport: "general"`, not `"football"`. Norwegian headlines like "Rødt kort-drama da Sørloth & co tapte" never match because of this. Fix: relax the 4th-tier to also accept `sport: "general"` items that contain football-related keywords, or remove the sport check entirely for the 4th tier (the 6h time proximity + team name match is already a strong enough guard). Files: `scripts/fetch-results.js` (matchRssHeadline). Pillar: quality. Scouted run 32 (data scout).
- [PENDING] [MAINTENANCE] **RSS item cap starves football-specific feeds** — `fetch-rss.js` caps digest at 30 items sorted by date, which fills with NRK/TV2 general content. Football-specific items from `tv2-fotball` and `bbc-football` (the only ones with `sport: "football"`) get dropped. Fix: ensure per-sport minimum retention (e.g. 5 items per sport type) before applying the cap, or increase cap to 50. This compounds with the 4th-tier sport filter bug — even if we relax the filter, there may be no football-tagged items to match against. Files: `scripts/fetch-rss.js`. Pillar: quality. Scouted run 32 (code scout).
- [PENDING] [MAINTENANCE] **Add Norwegian Cup (nor.cup) to football leagues** — RSS has 4+ stories about NM Cup semifinals but `nor.cup` is not in the football leagues config. Bodø/Glimt and potentially Lyn Oslo are in the cup — high Norwegian interest. Add `{ code: "nor.cup", name: "Norwegian Cup" }` to `scripts/config/sports-config.js` football sources. Pillar: data. Scouted run 32 (data scout).
