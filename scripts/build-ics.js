...
try { events = JSON.parse(fs.readFileSync(eventsFile,'utf-8')); } catch (e) {
  console.error('Failed to parse events.json:', e.message); process.exit(1);
}
if (!Array.isArray(events)) { console.error('events.json must be an array'); process.exit(1); }
...
for (const ev of events) {
  ...
  if (Array.isArray(ev.participants) && ev.participants.length) parts.push('Participants: '+ev.participants.join(', '));
  if (ev.streaming && ev.streaming.length) parts.push('Streaming: '+ev.streaming.map(s=>s.platform).join('/'));
